...
// ==============================================================
`timescale 1ns/1ps
module lzw_compress_hw_control_s_axi
#(parameter
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_TEMP_OUT_BUFFER_DATA_1: begin
                    rdata <= int_temp_out_buffer[63:32];
                end
                ADDR_TEMP_OUT_BUFFER_SIZE_DATA_0: begin
                    rdata <= int_temp_out_buffer_size[31:0];
                end
                ADDR_TEMP_OUT_BUFFER_SIZE_DATA_1: begin
                    rdata <= int_temp_out_buffer_size[63:32];
                end
            endcase
        end
    end
...
assign is_dup               = int_is_dup;
assign dup_index            = int_dup_index;
assign temp_out_buffer      = int_temp_out_buffer;
assign temp_out_buffer_size = int_temp_out_buffer_size;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
// int_temp_out_buffer[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_temp_out_buffer[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TEMP_OUT_BUFFER_DATA_0)
            int_temp_out_buffer[31:0] <= (WDATA[31:0] & wmask) | (int_temp_out_buffer[31:0] & ~wmask);
    end
end

// int_temp_out_buffer[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_temp_out_buffer[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TEMP_OUT_BUFFER_DATA_1)
            int_temp_out_buffer[63:32] <= (WDATA[31:0] & wmask) | (int_temp_out_buffer[63:32] & ~wmask);
    end
end

// int_temp_out_buffer_size[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_temp_out_buffer_size[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TEMP_OUT_BUFFER_SIZE_DATA_0)
            int_temp_out_buffer_size[31:0] <= (WDATA[31:0] & wmask) | (int_temp_out_buffer_size[31:0] & ~wmask);
    end
end

// int_temp_out_buffer_size[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_temp_out_buffer_size[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TEMP_OUT_BUFFER_SIZE_DATA_1)
            int_temp_out_buffer_size[63:32] <= (WDATA[31:0] & wmask) | (int_temp_out_buffer_size[63:32] & ~wmask);
    end
end
