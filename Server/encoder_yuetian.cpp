...
int main(int argc, char* argv[]) {
	...
	for (unsigned int i = 0; i < chunk_count; i++) {
		...
		// ËÆ°ÁÆóÂéãÁº©Êï∞ÊçÆÁö? SHA-256 ÂìàÂ∏å
		uint8_t temp_sha256_output[32]; // SHA-256 ËæìÂá∫‰∏? 32 Â≠óËäÇÁö? uint8_t Êï∞ÁªÑ
		...
		if(!isdup){
			...
			uint8_t real_output[compressed_size * 2];
			lzw(compressed_data, compressed_size, dict, temp_output, output_index);
			convert_output(temp_output, real_output, output_index);
		}
